...
class DomainSpanDecoder(nn.Module):
    ...
    def forward(self, inputs, hidden_states, dec_last_w, dec_last_h, first_turn, first_step, para_dec, mode='train'):
        gru_input = []
...
class BeliefSpanDecoder(nn.Module):
    def __init__(self, embedding, vocab_size_oov, bspn_mode, Wgen=None, dropout=0.0):
        ...
        self.gru = nn.GRU(4*cfg.hidden_size + self.embed_size, cfg.hidden_size, cfg.dec_layer_num,
                                     dropout=cfg.dropout, batch_first=True)
        init_gru(self.gru)
        ...
        self.attn_user = Attn(cfg.hidden_size)
        self.attn_para = Attn(cfg.hidden_size)
        self.attn_pvresp = self.attn_user if cfg.attn_param_share else Attn(cfg.hidden_size)
    ...
    # def forward(self, inputs, huser, hresp, hbspn, dec_last_w, dec_last_h, first_turn, first_step):
        ...
        # print('user:', inputs['user'][0:2, :])
        context_user = self.attn_user.forward(dec_last_h, hidden_states['user'], self.mask_user)
        context_para = self.attn_para.forward(dec_last_h, para_dec, self.mask_para)
        # context_user = self.attn_user(dec_last_h, huser, self.mask_user)
        gru_input.append(context_user)
        gru_input.append(context_para)
        # print(context_user.size())
        if not first_turn:
            context_pvresp = self.attn_pvresp.forward(dec_last_h, hidden_states['resp'], self.mask_pvresp)
            context_pvbspn = self.attn_pvbspn.forward(dec_last_h, hidden_states[self.bspn_mode], self.mask_pvbspn)
...
class ActSpanDecoder(nn.Module):
    ...
    def forward(self, inputs, hidden_states, dec_last_w, dec_last_h, first_turn, first_step, para_dec, bidx = None, mode='train'):
...
class DAMD(nn.Module):
    ...
    def forward(self, inputs, hidden_states, first_turn, mode, para_dec):
        if mode == 'train' or mode == 'valid':
            # probs, hidden_states = \
            probs = \
                self.train_forward(inputs, hidden_states, first_turn, para_dec)
            total_loss, losses = self.supervised_loss(inputs, probs)
            return total_loss, losses
        elif mode == 'test':
            decoded = self.test_forward(inputs, hidden_states, first_turn, para_dec)
            return decoded
        elif mode == 'rl':
            raise NotImplementedError('RL not available at the moment')


    def train_forward(self, inputs, hidden_states, first_turn, para_dec):
        """
        compute required outputs for a single dialogue turn. Turn state{Dict} will be updated in each call.
        """
        def train_decode(name, init_hidden, hidden_states, probs, para_dec, bidx=None):
            ...
            for t in range(decode_step):
                ...
                first_step = (t==0)
                if bidx is None:
                    dec_last_h = self.decoders[name].forward(inputs, hidden_states, dec_last_w,
                                                             dec_last_h, first_turn, first_step, para_dec)
        ...
        user_enc, user_enc_last_h = self.user_encoder.forward(inputs['user'])
        usdx_enc, usdx_enc_last_h = self.usdx_encoder.forward(inputs['usdx'])
        resp_enc, resp_enc_last_h = self.usdx_encoder.forward(inputs['pv_resp'])
        ...
        if cfg.enable_dspn:
            dspn_enc, _ = self.span_encoder.forward(inputs['pv_dspn'])
            hidden_states['dspn'] = dspn_enc
            hidden_states, probs = train_decode('dspn', usdx_enc_last_h, hidden_states, probs, para_dec)

        if cfg.enable_bspn:
            bspn_enc, _ = self.span_encoder.forward(inputs['pv_'+cfg.bspn_mode])
            hidden_states[cfg.bspn_mode] = bspn_enc
            init_hidden = user_enc_last_h if cfg.bspn_mode == 'bspn' else usdx_enc_last_h
            hidden_states, probs = train_decode(cfg.bspn_mode, init_hidden, hidden_states, probs, para_dec)

        if cfg.enable_aspn:
            aspn_enc, _ = self.span_encoder.forward(inputs['pv_aspn'])
            hidden_states['aspn'] = aspn_enc
            hidden_states, probs = train_decode('aspn', usdx_enc_last_h, hidden_states, probs, para_dec)

        hidden_states, probs = train_decode('resp', usdx_enc_last_h, hidden_states, probs, para_dec)

        if cfg.enable_dst and cfg.bspn_mode == 'bsdx':
            bspn_enc, _ = self.span_encoder.forward(inputs['pv_bspn'])
            hidden_states['bspn'] = bspn_enc
            hidden_states, probs = train_decode('bspn', user_enc_last_h, hidden_states, probs, para_dec)
        ...
        if cfg.enable_aspn and cfg.multi_acts_training and 'aspn_aug' in inputs:
            ...
            for b in range(len(inputs['aspn_bidx'])//batch_size+1):
                ...
                if bidx_batch:
                    ...
                    _, ps = train_decode('aspn', usdx_enc_last_h, hidden_states, None, para_dec, bidx=bidx_batch)
                    probs['aspn_aug'].append(ps)

        return probs


    def test_forward(self, inputs, hs, first_turn, para_dec):
        user_enc, user_enc_last_h = self.user_encoder.forward(inputs['user'])
        usdx_enc, usdx_enc_last_h = self.usdx_encoder.forward(inputs['usdx'])
        resp_enc, resp_enc_last_h = self.usdx_encoder.forward(inputs['pv_resp'])
        hs['user'] = user_enc
        ...
        if cfg.enable_dst and cfg.bspn_mode == 'bsdx':
            bspn_enc, _ = self.span_encoder.forward(inputs['pv_bspn'])
            hs['bspn'] = bspn_enc
            hs, decoded = self.greedy_decode('bspn', user_enc_last_h, first_turn, inputs, hs, decoded, para_dec)

        if cfg.enable_dspn:
            dspn_enc, dspn_enc_last_h = self.span_encoder.forward(inputs['pv_dspn'])
            hs['dspn'] = dspn_enc
            hs, decoded = self.greedy_decode('dspn', usdx_enc_last_h, first_turn, inputs, hs, decoded, para_dec)

        if cfg.enable_bspn:
            bspn_enc, bspn_enc_last_h = self.span_encoder.forward(inputs['pv_'+cfg.bspn_mode])
            hs[cfg.bspn_mode] = bspn_enc
            init_hidden = user_enc_last_h if cfg.bspn_mode == 'bspn' else usdx_enc_last_h
            hs, decoded = self.greedy_decode(cfg.bspn_mode, init_hidden, first_turn, inputs, hs, decoded, para_dec)
        ...
        if cfg.enable_aspn:
            aspn_enc, aspn_enc_last_h = self.span_encoder.forward(inputs['pv_aspn'])
            hs['aspn'] = aspn_enc
            if cfg.aspn_decode_mode == 'greedy':
                hs, decoded = self.greedy_decode('aspn', usdx_enc_last_h, first_turn, inputs, hs, decoded, para_dec)
            elif cfg.aspn_decode_mode == 'beam':
                if cfg.record_mode:
                    hs_nbest, decoded_nbest = self.beam_decode('aspn', usdx_enc_last_h, first_turn, inputs, hs, decoded,
                                                               para_dec)
                else:
                    hs, decoded = self.beam_decode('aspn', usdx_enc_last_h, first_turn, inputs, hs, decoded, para_dec)
            elif 'sampling' in cfg.aspn_decode_mode:
                hs, decoded = self.sampling_decode('aspn', usdx_enc_last_h, first_turn, inputs, hs, decoded, para_dec)
        ...
        if cfg.record_mode:
            ...
            for i in range(cfg.nbest):
                ...
                hs['aspn'] = hs_nbest[:, i, :, :]
                hs, decoded = self.greedy_decode('resp', usdx_enc_last_h, first_turn, inputs, hs, decoded, para_dec)
    ...
    def RL_forward(self, inputs, decoded, hiddens_batch, decoded_batch):
        ...
        """
        batch_size = hiddens_batch.size()[0]
        logprob = self.act_selection.forward(hiddens_batch)   #[B, nbest]
    ...
    def greedy_decode(self, name, init_hidden, first_turn, inputs, hidden_states, decoded, para_dec):
        ...
        for t in range(max_len):
            ...
            first_step = (t==0)
            dec_last_h = self.decoders[name].forward(inputs, hidden_states, dec_last_w,
                                                     dec_last_h, first_turn, first_step, para_dec, mode='test')
    ...
    def beam_decode(self, name, init_hidden, first_turn, inputs, hidden_states, decoded, para_dec):
    ...
    def sampling_decode(self, name, init_hidden, first_turn, inputs, hidden_states, decoded, para_dec):
        max_len = cfg.max_nl_length if name == 'resp' else cfg.max_span_length
        batch_size = inputs['user'].size(0)
    ...
    def RL_train(self, inputs, hs, hiddens_batch, decoded_batch, first_turn, para_dec):
        """[summary]
        :param hiddens_batch: [B, nbest, T, H]
        :param decoded_batch: [B, nbest, T]
        """
...
class SimpleDynamicEncoder(nn.Module):
    ...
    def forward(self, input_seqs, input_lens, hidden=None):
        ...
        unsort_idx = cuda_(torch.from_numpy(np.argsort(sort_idx)).type(torch.LongTensor))
        sort_idx = cuda_(torch.from_numpy(sort_idx).type(torch.LongTensor))
...
class Attn_Para(nn.Module):
    ...
    def forward(self, hidden, encoder_outputs, mask=False, inp_seqs=None, stop_tok=None, normalize=True):
        ...
        if True or not mask:
            ...
            mask = cuda_(torch.FloatTensor(mask))  # [B,T]
...
class ActDecoder(nn.Module):
    def __init__(self, embed_size, hidden_size, vocab_size, dropout_rate, vocab):
        ...
        self.attn_u = Attn_Para(hidden_size)
...
class ParaDecoder(nn.Module):
    def __init__(self, embed_size, hidden_size, vocab_size, dropout_rate, vocab):
        ...
        self.attn_u = Attn_Para(hidden_size)
        self.attn_a = Attn_Para(hidden_size)
        self.proj_copy = nn.Linear(hidden_size, hidden_size)
        self.dropout_rate = dropout_rate
...
class Paraphrase(nn.Module):
    def __init__(self, reader):
        ...
        self.reader = reader
        self.vocab = self.reader.vocab
        self.emb = nn.Embedding(self.vocab.vocab_size, cfg.embed_size)
        self.proj = nn.Linear(cfg.hidden_size * 3, self.vocab.vocab_size)
        self.u_encoder = SimpleDynamicEncoder(self.vocab.vocab_size, cfg.embed_size, cfg.hidden_size, cfg.enc_layer_num,
                                              cfg.dropout)
        self.a_decoder = ActDecoder(cfg.embed_size, cfg.hidden_size, self.vocab.vocab_size, cfg.dropout, self.vocab)
        self.p_decoder = ParaDecoder(cfg.embed_size, cfg.hidden_size, self.vocab.vocab_size, cfg.dropout, self.vocab)

        self.embed_size = cfg.embed_size

        self.max_para_len = cfg.max_nl_length
        self.a_length = cfg.max_span_length
        self.teacher_force = cfg.teacher_force
    ...
    def forward_turn(self, u_input, u_len, mode, u_input_np, sparse_u_input_para, para_input=None, prev_act_input=None):
        ...
        a_tml = cuda_(torch.ones(1, batch_size).long())
        p_tm1 = cuda_(torch.ones(1, batch_size).long())
        ...
        if mode == 'train':
            ...
            for t in range(cfg.a_length):
                ...
                for i in range(a_index.size(0)):
                    if a_index[i] >= cfg.vocab_size:
                        a_index[i] = 2  # unk
                a_tml = cuda_(a_index.view(1, -1))
            ...
            for t in range(cfg.max_para_len):
                ...
                for i in range(parat_index.size(0)):
                    if parat_index[i] >= cfg.vocab_size:
                        parat_index[i] = 2  # unk
                p_tm1 = cuda_(parat_index.view(1, -1))
    ...
    def para_decode(self, u_enc_out, p_tm1, u_input_np, last_hidden, sparse_u_input_para, a_enc_out, a_input_np):
        ...
        for t in range(cfg.max_para_len):
            ...
            for i in range(parat_index.size(0)):
                if parat_index[i] >= cfg.vocab_size:
                    parat_index[i] = 2  # unk
            u_tm1 = cuda_(parat_index.view(1, -1))
        decoded = torch.stack(decoded, dim=0).transpose(0, 1)
        decoded = list(decoded)
