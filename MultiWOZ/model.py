...
from damd_net import DAMD, cuda_, get_one_hot_input, Paraphrase
from eval import MultiWozEvaluator
from damd_net import get_sparse_input_aug
from para_analysis import realization_multiwoz, slots_match_multiwoz
...
class Model(object):
    def __init__(self):
        ...
        if len(cfg.cuda_device)==1:
            ...
            self.m_para = Paraphrase(self.reader)
        else:
            m = DAMD(self.reader)
            m_para = Paraphrase(self.reader)
            self.m = torch.nn.DataParallel(m, device_ids=cfg.cuda_device)
            self.m_para = torch.nn.DataParallel(m_para, device_ids=cfg.cuda_device)
            # print(self.m.module)
    ...
    def _convert_batch_para(self, py_batch, mode, prev_a_py=None):
        combined_input = []
        combined_length = []
        for prev_response, delex_user in zip(py_batch['pv_resp'], py_batch['usdx']):
            combined_input.append(prev_response + delex_user)
            combined_length.append(len(prev_response + delex_user))
        u_input_np = pad_sequences(combined_input, cfg.max_nl_length, padding='post',
                                   truncating='pre').transpose((1, 0))
        u_len = np.array(combined_length)
        delex_para_input_np = pad_sequences(py_batch['padx'], cfg.max_nl_length, padding='post',
                                            truncating='pre').transpose((1, 0))
        ...
        if mode == 'test':
            ...
            prev_dial_act_input_np = pad_sequences(prev_a_py, cfg.max_nl_length, padding='post',
                                                   truncating='pre').transpose((1, 0))
            prev_dial_act_input = cuda_(torch.from_numpy(prev_dial_act_input_np).long())
        else:
            prev_dial_act_input_np = pad_sequences(py_batch['pv_aspn'], cfg.max_nl_length, padding='post',
                                                   truncating='pre').transpose((1, 0))
            prev_dial_act_input = cuda_(torch.from_numpy(prev_dial_act_input_np).long())
    ...
    def _get_final_input(self, py_batch, para_results, epoch):
        ...
        if epoch % 2 == 0:
            for i in range(batch_size):
                ...
                final_user_delex.append(delex_user[i])
        else:
            user_delex = py_batch['usdx']
            user_utter = []
            delex_user_utter = []
            para_utter = []
            slu = py_batch['slu']
            ...
            for i in range(batch_size):
                user_utter.append(self.reader.vocab.sentence_decode(user[i]))
                delex_user_utter.append(self.reader.vocab.sentence_decode(user_delex[i]))
                para_utter.append(self.reader.vocab.sentence_decode(para[i]))
            ...
            for i in range(batch_size):
                ...
                if success[i] and slots_match_success[i] and sentence_bleu([s1, s2], s) > cfg.bleu_threshold:
                    good += 1
                    select = self.reader.vocab.sentence_encode(gen_para[i].split() + ['<eos_u>'])
                    delex_select = self.reader.vocab.sentence_encode(para_results[i].split() + ['<eos_u>'])
                    final_user.append(select)
                    final_user_delex.append(len(delex_select))
                else:
                    final_user.append(user[i])
                    final_user_delex.append(delex_user[i])
                total += 1
        ...
        py_batch['final_usdx'] = final_user_delex
        return py_batch, weight

    def add_torch_input(self, inputs, mode='train', first_turn=False):
        need_onehot = ['final_user', 'final_usdx', 'bspn', 'aspn', 'pv_resp', 'pv_bspn', 'pv_aspn',
                                   'dspn', 'pv_dspn', 'bsdx', 'pv_bsdx']
        inputs['db'] = cuda_(torch.from_numpy(inputs['db_np']).float())
        for item in ['final_user', 'final_usdx', 'resp', 'bspn', 'aspn', 'bsdx', 'dspn']:
            ...
            if item in ['final_user', 'final_usdx', 'resp', 'bspn']:
            ...
            if item in ['resp', 'bspn', 'aspn', 'bsdx', 'dspn']:
                ...
                if item in ['final_user', 'final_usdx', 'bspn']:
        ...
        for epoch in range(cfg.epoch_num):
            ...
            for iter_num, dial_batch in enumerate(data_iterator):
                ...
                for turn_num, turn_batch in enumerate(dial_batch):
                    ...
                    sparse_u_input_para = get_sparse_input_aug(u_input_np)
                    ...
                    dial_total_loss, losses = self.m(inputs, hidden_states, first_turn, mode='train',
                    ...
                    total_loss = loss_para + dial_total_loss.mean()
        ...
        for batch_num, dial_batch in enumerate(data_iterator):
            ...
            for turn_num, turn_batch in enumerate(dial_batch):
                ...
                if cfg.valid_loss not in ['score', 'match', 'success', 'bleu']:
                    valid_loss += float(loss_para)
                    total_loss, losses = self.m(inputs, hidden_states, first_turn, mode='train', para_dec=para_dec_outs)
                    py_prev['pv_resp'] = turn_batch['resp']
                    if cfg.enable_bspn:
                        py_prev['pv_bspn'] = turn_batch['bspn']
                        py_prev['pv_bsdx'] = turn_batch['bsdx']
                    ...
                    decoded = self.m(inputs, hidden_states, first_turn, mode='test', para_dec=para_dec_outs)
    ...
    def eval(self, data='test'):
        ...
        for batch_num, dial_batch in tqdm.tqdm(enumerate(data_iterator)):
            ...
            for turn_num, turn_batch in enumerate(dial_batch):
                ...
                sparse_u_input_para = Variable(get_sparse_input_aug(u_input_np), requires_grad=False)
                para_dec_outs, _, prev_act_idx = self.m_para(u_input=u_input,
                                                             para_input=para_input,
                                                             u_input_np=u_input_np,
    ...
    def save_model(self, epoch, path=None, path_para=None, critical=False):
        if not cfg.save_log:
            return
        if not path:
            path = cfg.model_path
        if not path_para:
            path_para = cfg.para_model_path
        if critical:
            path += '.final'
            path_para += '.final'
        all_state = {'lstd': self.m.state_dict(),
                     'config': cfg.__dict__,
                     'epoch': epoch}
        all_state_para = {'lstd': self.m_para.state_dict(),
                          'config': cfg.__dict__,
                          'epoch': epoch}
        torch.save(all_state, path)
        torch.save(all_state_para, path_para)
        logging.info('Model saved')

    def load_model(self, path=None, path_para=None):
        if not path:
            path = cfg.model_path
        if not path_para:
            path_para = cfg.para_model_path
        ...
        all_state_para = torch.load(path_para, map_location='cpu')
        self.m_para.load_state_dict(all_state_para['lstd'])
...
def main():
    ...
    if args.mode == 'test' or args.mode=='adjust':
        ...
        for k, v in cfg_load.items():
            ...
            cfg.para_model_path = os.path.join(cfg.eval_load_path, 'para_model.pkl')
    else:
        parse_arg_cfg(args)
